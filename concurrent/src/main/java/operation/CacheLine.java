...
/**
 * 在计算机系统中,内存是以缓存行为单位进行存储的,缓存行的大小为从32个自己到256个字节不等每一个CPU都有对应的CPU缓存
 * 伪共享是指不同的线程读到同一个缓存行上的不同的变量
 */
public class CacheLine {
    ...
    // 有填充、有缓存行对齐的对象类, 对象头占用8个字节,再加上value8个字节以及6个用于填充的long类型的对象的48个字节,正好64个字节
    // 这些字段是有可能在同一个缓存行的
